...
const createMatchSchema = z.object({
  ...
  status: z.enum(['pending', 'accepted', 'rejected', 'contacted']).default('pending')
})
...
router.post('/generate', async (req, res) => {
  try {
    ...
    const matchingProjects = await prisma.agriTechStartup.findMany({
      where: {
        AND: [
          ...
          // Exclude projects already in portfolio or matched
    ...
    for (const project of matchingProjects) {
      const score = calculateMatchScore(investor, project)
      const reasons = generateMatchReasons(investor, project)
...
function calculateMatchScore(investor: any, startup: any): number {
  ...
  if (investor.preferredIndustries.includes(project.landType)) {
    score += 30
  }
  factors++
  ...
  if (project.fundingRequired && 
      project.fundingRequired >= investor.investmentRangeMin && 
      project.fundingRequired <= investor.investmentRangeMax) {
    score += 5
  }
  factors++
...
function generateMatchReasons(investor: any, startup: any): string[] {
  ...
  if (investor.preferredIndustries.includes(project.landType)) {
    reasons.push(`Perfect industry alignment (${project.landType})`)
  }

  if (investor.preferredStages.includes(project.stage)) {
    reasons.push(`Stage (${project.stage}) matches your investment focus`)
  }

  if (project.techScore >= investor.techScoreMin) {
    reasons.push(`Strong tech score (${project.techScore}/100) meets your minimum (${investor.techScoreMin}+)`)
  }

  if (project.socialScore >= investor.socialScoreMin) {
    reasons.push(`Good social score (${project.socialScore}/100) meets your minimum (${investor.socialScoreMin}+)`)
  }

  if (project.teamSize >= investor.teamSizeMin) {
    reasons.push(`Team size (${project.teamSize}) exceeds your preference (${investor.teamSizeMin}+)`)
  }

  const foundedYear = new Date(project.createdAt).getFullYear()
  if (foundedYear >= investor.foundedYearMin) {
    reasons.push(`Founded in ${foundedYear}, meets your minimum (${investor.foundedYearMin}+)`)
  }
